    ...
    try {
        ...
        $stmt->execute([$_SESSION['usrid']]);
         ...
         //   var_dump($name,$value);
           ...
           //插入操作
            $stmt = $DBH->prepare("INSERT into award(username, realname, lotteryname, authority, type1, award1, type2, award2, type3, award3, type4, award4, type5, award5, type6, award6, type7, award7, type8, award8, type9, award9, type10, award10 ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
            $stmt->execute([$username, $realname, $lotteryname, $authority, $value[0], $name[0], $value[1], $name[1], $value[2], $name[2], $value[3], $name[3],
